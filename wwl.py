...
# Embedding schemes
####################
def compute_wl_embeddings_continuous(node_features, adj_mat, edge_features, n_nodes, h):
    '''
    ...
    labels_sequence = create_labels_seq_cont(node_features, adj_mat, h, 
                        None if len(edge_features)==0 else edge_features)

    return labels_sequence

def compute_wl_embeddings_discrete(adj_mat, node_labels, h):
    graphs = []
    assert(len(adj_mat)==len(node_labels))
    for i in tqdm(range(len(adj_mat))):
        g = ig.Graph.Adjacency(adj_mat[i].tolist())
        g.vs['label'] = [str(int(i)) for i in node_labels[i].reshape(-1)]
        graphs.append(g)

    wl = WeisfeilerLehman()
    label_dicts = wl.fit_transform(graphs, h) # dict shape = (h+1, num_graph)
...
class WeisfeilerLehman(TransformerMixin):
    ...
    def _relabel_graphs(self, X: List[ig.Graph]):
        r"""
        relabel: let labels index from 0 to ~
        """
        num_unique_labels = 0
        preprocessed_graphs = []
        for i, g in enumerate(X):
            ...
            labels = x.vs['label'] # label for each node in grap x, discrete
    ...
    def fit_transform(self, X: List[ig.Graph], num_iterations: int=3):
        ...
        for it in np.arange(1, num_iterations+1, 1):
            ...
            for i, g in enumerate(X):
                ...
                merged_labels = [[b]+a for a,b in zip(neighbor_labels, current_labels)] # stack each node's label with its neihbors'

                # Generate a label dictionary based on the merged labels
                self._append_label_dict(merged_labels) #map the merged label into index label, get a dict

                # Relabel the graph
                new_labels = self._relabel_graph(g, merged_labels) # using the generated dict to map each node's merged label into new index label
    ...
    def _get_neighbor_labels(self, X: ig.Graph, sort: bool=True):
        r"""
        compute neihbor's labels of each node, then return the sorted labels
        """
    ...
    for h in hs:
        ...
        for graph_index_1, graph_1 in enumerate(label_sequences):
            ...
            for graph_index_2, graph_2 in enumerate(label_sequences[graph_index_1:]):
                ...
                costs = ot.dist(labels_1, labels_2, metric=ground_distance) # use 0-1 code to represent each label, and compute the distance between each label, shape = (num_labels_1, num_labels_2)
